...
class UserManager:
    def __init__(self, db):
        ...
        self.USER_TABLE_COL_TYPES = ["integer primary key", "TEXT NOT NULL", 
    ...
    def add_user(self, name, password, apikey, apisecret):
        self.__db.insert_to_table(self.USER_TABLE_NAME, self.USER_TABLE_COLS, ["null", f"'{name}'", 
                                f"'{password}'",f"'{apikey}'", f"'{apisecret}'"])
    ...
    def _create_default_user(self):
        data = self.__db.select_from_table(self.USER_TABLE_NAME, ["name", "pass"], ["name='giant'", "pass='giant'"])
        if len(data) == 0:
            self.add_user("giant", "giant", "api_key", "api_secret")
