...
# exit(0)
train_x = np.array([img for img in train_x])
...
val_x = np.array([img for img in val_x])
print("\nBEFORE \n",val_y)
...
model.load_weights("data/private/weights/1-29-0.662-0.621-0.667-0.621.hdf5")
history = []
# for _ in range(2):
...
history = model.fit(train_x, train_y, epochs=EPOCHS, verbose=1, validation_data=(val_x, val_y), callbacks=[checkpoint, show_val_stats], batch_size=256)
...
# plt.savefig(f"data/private/img/accs-{time.time()}.png")
# plt.show()
...
# plt.xlabel('Epoch')
# plt.legend(['Train', 'Test'], loc='upper left')
# plt.savefig(f"data/private/img/losses-{time.time()}.png")
# plt.show()
