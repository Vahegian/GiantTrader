...
of multiple cryptocurrencies and produces 3 other files "train", "test" and "validate".
These files contain 4x7 images of the markets and buy or sell signals associated with the 
images. Use 'train' and 'validate' files to train a neural network and use 'test' to test the trained 
network.
'''
...
train_size, test_size = int((80/100)*len(data)), int((10/100)*len(data)) # 90%, 5%
train, test, val = data[:train_size], data[train_size:train_size+test_size], data[train_size+test_size:]
...
p2.join()
p3.join()        
# get_OHLC_bin_images(train, "data/private/train.npy")